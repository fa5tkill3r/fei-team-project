<template>
  <div>
    <PageTitle :text="$t('admin_panel.incident_categories')" />

    <h1 class="text-2xl mb-2 font-bold">
      {{ $t('admin_panel.incident_categories') }}
    </h1>
...
function deleteCategory() {
  if (deleteCategoryId.value) {
    auth.client
      .delete(`incident-categories/${deleteCategoryId.value}`)
      .json()
      .then(() => {
...
function create() {
  ...
  auth.client
    .post({ name: newCategoryName.value }, 'incident-categories')
    .json()
    .then(({ data }: any) => {
...
function save(event: MouseEvent, id: number) {
  ...
  auth.client
    .put({ name }, `incident-categories/${id}`)
    .json()
    .then(() => {
